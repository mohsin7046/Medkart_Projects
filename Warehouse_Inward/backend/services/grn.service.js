...
export const createGRNRecord = async (existingPO, data) => {
  ...
  for (const poItem of existingPO.purchaseOrderItems) {
    ...
    if (!receivedItem) {
      throw new Error(`Received item with product code ${poItem.product_code} not found in PO items`);
    }
...
export const getALLGRNService = async () => {
  const getALLGRN = await prisma.goodReceiptNote.findMany({
    include: { goodReceiptNoteItems: true },
