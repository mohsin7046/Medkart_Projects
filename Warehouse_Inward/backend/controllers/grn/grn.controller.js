...
export const createGRN = async (req, res) => {
  ...
  try {
    ...
    if (['completed', 'cancelled'].includes(existingPO.status)) {
      return res
        .status(400)
        .json({ error: 'GRN already created for this order' })
    }
    ...
    for (const poItem of existingPO.purchaseOrderItems) {
      const receivedItem = items.find(
        (i) => i.product_code === poItem.product_code
      )
      if (!receivedItem) {
        return res
          .status(400)
          .json({
            error: `Received item with product code ${poItem.product_code} not found in PO items`
          })
      }
    ...
    if (statusUpdate === 'cancelled') {
      ...
      return res
        .status(400)
        .json({ error: 'Received quantity or MRP is not valid' })
    }
    ...
    if (!updatePO) {
      return res
        .status(500)
        .json({ error: 'Failed to update Purchase Order status' })
    }
...
export const updateGRN = async (req, res) => {
  try {
    ...
    if (['completed', 'cancelled'].includes(existingGRN.status)) {
      return res
        .status(400)
        .json({ error: 'Cannot update a completed or cancelled GRN' })
    }
